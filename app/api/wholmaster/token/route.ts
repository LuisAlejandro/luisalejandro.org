...
const WRITE_KEY = process.env.WHOLMASTER_COSMIC_WRITE_KEY;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authHeader) {
      console.log(`[wholmaster-token-api] Missing authorization header`);
    ...
    if (!isSchemaValid(schema, body)) {
      console.log(`[wholmaster-token-api] Invalid body`);
      return NextResponse.json(
        { error: "Invalid body" },
        { status: 400, headers: corsHeaders }
      );
    ...
    if (!isAuthorizationValid(authHeader, body)) {
      console.log(`[wholmaster-token-api] Invalid authorization`);
      return NextResponse.json(
    ...
    if (data.objects.length === 0) {
      console.log(`[wholmaster-token-api] User not found`);
      return NextResponse.json(
    ...
    if (!isValidPassword) {
      console.log(`[wholmaster-token-api] Unauthorized: password`);
      return NextResponse.json(
    ...
    if (!storedIdentifier) {
      ...
      console.log(`[wholmaster-token-api] Unauthorized: identifier`);
      return NextResponse.json(
    ...
    console.log(`[wholmaster-token-api] Token generated`);
