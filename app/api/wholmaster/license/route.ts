...
const ACTIVATION_SECRET = process.env.WHOLMASTER_ACTIVATION_SECRET;

const ACTIVATION_SALT = process.env.WHOLMASTER_ACTIVATION_SALT;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!token) {
      console.log(`[wholmaster-license-api] Missing authorization header`);
    ...
    if (!isSchemaValid(schema, body)) {
      console.log(`[wholmaster-license-api] Invalid body`);
      return NextResponse.json(
        { error: "Invalid body" },
        { status: 400, headers: corsHeaders }
      );
    ...
    if (!decodedToken) {
      console.log(`[wholmaster-license-api] Unauthorized: token`);
      return NextResponse.json(
    ...
    if (!isValidPassword) {
      console.log(
        `[wholmaster-license-api] Unauthorized: password ${decodedToken.username}`
      );
      return NextResponse.json(
    ...
    if (
      ...
      if (!decodedKey) {
        console.log(
          `[wholmaster-license-api] Unauthorized: key ${decodedToken.username}`
        );
        return NextResponse.json(
      ...
      console.log(
        `[wholmaster-license-api] Authorized: key ${decodedToken.username}`
      );
      return NextResponse.json(
        {
    ...
    console.log(
      `[wholmaster-license-api] Unauthorized: salt ${decodedToken.username}`
    );
    return NextResponse.json(
    ...
    logError("wholmaster-license-api", error, {
      bucketSlug: BUCKET_SLUG || "",
      hasReadKey: !!READ_KEY,
